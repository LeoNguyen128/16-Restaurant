...
<body >
    ...
    <main>
        <section class="container mb-5 bg-light align-items-center" style="min-height:10px;">
            ...
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h2>Customer Going Together</h2>
                    <table class="table table-striped text-center  table-hover">
                        <thead class="thead-dark">
                            <tr>
                                <th>Processing Reservation and Order</th>
                                <th>Finished Reservation and Order</th>
                            </tr>
                        </thead>
                        <?php 
                            global $ConnectingDB;
                            $sql1 = "SELECT COUNT(*) FROM reservation WHERE Stage='Processing'";
                            $sql2 = "SELECT COUNT(*) FROM reservation WHERE Stage='Finished'";
                            $stmt1=$ConnectingDB->query($sql1);
                            $stmt2=$ConnectingDB->query($sql2);
                            $rowsTotal=$stmt1->fetch();
                            $rowsTotal1=$stmt2->fetch();
                            $total=array_shift($rowsTotal);  
                            $total1=array_shift($rowsTotal1);  
                        ?>
